...
return { 
    ...
    execute = function(domoticz)
        ...
        if  domoticz.devices(71).state    == "Off"     then --  Presence detection 
			...
			-- Turn on Cam when we are away
      -- This switch are not in use as I'm using motioneyeos on a rasp
      -- This will be update in the future
--			domoticz.devices(34).switchOn()   --  Hallway Camera
--			print("Bye Bye")
        end
		
		
--		if  domoticz.devices(16).state    == "On"     then --  Presence detection 
		    -- Turn off cam when we are home
--			domoticz.devices(34).switchOff()   --  Hallway Camera
--		end
